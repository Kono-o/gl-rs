use super::*;
use crate::gl46::GLint;
use crate::{gl46 as ngl, version::Version};
use std::ffi::CStr;
...
impl Context {
   pub(crate) fn from(raw: ngl::GlFns, version: Version) -> Context {
      Context {
         raw,
         version,
   ...
   pub fn load<F: FnMut(&str) -> *const c_void>(
      ...
      unsafe {
         let raw = ngl::GlFns::load_with(|p: *const c_char| {
            let c_str = CStr::from_ptr(p).to_str().unwrap_or("");
            load_fn(c_str) as *mut c_void
         });

         let version_raw_str = raw.GetString(VERSION);
         if version_raw_str.is_null() {
            return Err(ContextInitError::NoActiveContext);
         }
         let version = match Version::parse(
            &CStr::from_ptr(version_raw_str as *const ngl::GLchar)
               .to_str()
               .unwrap_or("")
               .to_string(),
         ) {
            Err(s) => return Err(ContextInitError::CouldParseVersion(s.to_string())),
            Ok(v) => v,
         };
         let mut context = Context::from(raw, version);
...
#[derive(Copy, Clone, Debug, Eq, Hash, Ord, PartialEq, PartialOrd)]
pub struct NativeFence(pub ngl::GLsync);
...
#[derive(Copy, Clone, Debug, Eq, Hash, Ord, PartialEq, PartialOrd)]
pub struct NativeUniformLocation(pub ngl::GLuint);

#[derive(Copy, Clone, Debug, Eq, Hash, Ord, PartialEq, PartialOrd)]
pub struct NativeTransformFeedback(pub NonZeroU32);

impl __private::Sealed for Context {}
...
impl HasContext for Context {
   ...
   unsafe fn create_shader(&self, shader_type: u32) -> Result<Self::Shader, String> {
      let gl = &self.raw;
      NonZeroU32::new(gl.CreateShader(shader_type))
         .map(NativeShader)
   ...
   unsafe fn shader_source(&self, shader: Self::Shader, source: &str) {
      ...
      gl.ShaderSource(
         shader.0.get(),
         1,
         &(source.as_ptr() as *const ngl::GLchar),
         &(source.len() as ngl::GLint),
      );
   }
   ...
   unsafe fn get_program_binary(&self, program: Self::Program) -> Option<ProgramBinary> {
      ...
      let mut len = 0;
      gl.GetProgramiv(program.0.get(), PROGRAM_BINARY_LENGTH, &mut len);
   ...
   unsafe fn program_binary(&self, program: Self::Program, binary: &ProgramBinary) {
      ...
      gl.ProgramBinary(
         ...
         binary.buffer.len() as ngl::types::GLsizei,
      )
   }
   ...
   unsafe fn get_frag_data_location(&self, program: Self::Program, name: &str) -> i32 {
      ...
      gl.GetFragDataLocation(program.0.get(), name.as_ptr() as *const ngl::GLchar)
   }
   ...
   unsafe fn bind_frag_data_location(&self, program: Self::Program, color_number: u32, name: &str) {
      ...
      gl.BindFragDataLocation(
         program.0.get(),
         color_number,
         name.as_ptr() as *const ngl::GLchar,
      );
   }

   unsafe fn buffer_data_size(&self, target: u32, size: i32, usage: u32) {
      let gl = &self.raw;
      gl.BufferData(target, size as isize, ptr::null(), usage);
   }

   unsafe fn named_buffer_data_size(&self, buffer: Self::Buffer, size: i32, usage: u32) {
      let gl = &self.raw;
      gl.NamedBufferData(buffer.0.get(), size as isize, ptr::null(), usage);
   }
   ...
   unsafe fn buffer_storage(&self, target: u32, size: i32, data: Option<&[u8]>, flags: u32) {
      ...
      let data = data.map(|p| p.as_ptr()).unwrap_or(ptr::null()) as *const std::ffi::c_void;
   ...
   unsafe fn draw_arrays_instanced_base_instance(
      ...
      let gl = &self.raw;
      gl.DrawArraysInstancedBaseInstance(mode, first, count, instance_count, base_instance);
   }
   ...
   unsafe fn draw_elements(&self, mode: u32, count: i32, element_type: u32, offset: i32) {
      let gl = &self.raw;
      gl.DrawElements(mode, count, element_type, offset as *const std::ffi::c_void);
   }
   ...
   unsafe fn get_parameter_indexed_string(&self, parameter: u32, index: u32) -> String {
      ...
      CStr::from_ptr(raw_ptr as *const ngl::GLchar)
         .to_str()
         .unwrap()
         .to_owned()
   }
   ...
   unsafe fn get_parameter_renderbuffer(&self, parameter: u32) -> Option<Self::Renderbuffer> {
      self
         .get_parameter_gl_name(parameter)
         .map(NativeRenderbuffer)
   }
   ...
   unsafe fn get_active_uniform_block_name(
      ...
      let len = if gl.GetError() == NO_ERROR && len > 0 {
         len as usize
      } else {
         256
      };
   ...
   unsafe fn get_uniform_location(
      ...
      let uniform_location =
         gl.GetUniformLocation(program.0.get(), name.as_ptr() as *const ngl::GLchar);
   ...
   unsafe fn get_attrib_location(&self, program: Self::Program, name: &str) -> Option<u32> {
      ...
      let name = CString::new(name).unwrap();
      let attrib_location =
         gl.GetAttribLocation(program.0.get(), name.as_ptr() as *const ngl::GLchar);
   ...
   unsafe fn bind_attrib_location(&self, program: Self::Program, index: u32, name: &str) {
      let gl = &self.raw;
      let name = CString::new(name).unwrap();
      gl.BindAttribLocation(program.0.get(), index, name.as_ptr() as *const ngl::GLchar);
   }
   ...
   unsafe fn fence_sync(&self, condition: u32, flags: u32) -> Result<Self::Fence, String> {
      let gl = &self.raw;
      Ok(NativeFence(gl.FenceSync(condition, flags)))
   }
   ...
   unsafe fn blend_equation_draw_buffer(&self, draw_buffer: u32, mode: u32) {
      let gl = &self.raw;
      gl.BlendEquationi(draw_buffer, mode);
   }
   ...
   unsafe fn blend_equation_separate_draw_buffer(
      ...
      let gl = &self.raw;
      gl.BlendEquationSeparatei(draw_buffer, mode_rgb, mode_alpha);
   }
   ...
   unsafe fn blend_func_draw_buffer(&self, draw_buffer: u32, src: u32, dst: u32) {
      let gl = &self.raw;
      gl.BlendFunci(draw_buffer, src, dst);
   }
   ...
   unsafe fn blend_func_separate_draw_buffer(
      ...
      let gl = &self.raw;
      gl.BlendFuncSeparatei(draw_buffer, src_rgb, dst_rgb, src_alpha, dst_alpha);
   }
   ...
   unsafe fn stencil_op_separate(&self, face: u32, stencil_fail: u32, depth_fail: u32, pass: u32) {
      let gl = &self.raw;
      gl.StencilOpSeparate(face, stencil_fail, depth_fail, pass);
   }
      ...
      S: AsRef<str>,
   {
      ...
      gl.DebugMessageInsert(
         ...
         severity,
         length,
         message.as_ptr() as *const ngl::GLchar,
      );
   }
   ...
   unsafe fn get_debug_message_log(&self, count: u32) -> Vec<DebugMessageLogEntry> {
      ...
      let mut offset = 0;
      for i in 0..received {
         let message = CStr::from_ptr(message_log[offset..].as_ptr()).to_string_lossy();
      ...
      S: AsRef<str>,
   {
      ...
      gl.PushDebugGroup(source, id, length, msg.as_ptr() as *const ngl::GLchar);
   }
      ...
      S: AsRef<str>,
   {
      ...
      match label {
         Some(l) => {
            ...
            gl.ObjectLabel(identifier, name, length, lbl.as_ptr() as *const ngl::GLchar);
         }
         None => gl.ObjectLabel(identifier, name, 0, ptr::null()),
      }
   }
   ...
   unsafe fn get_object_label(&self, identifier: u32, name: u32) -> String {
      ...
      CStr::from_ptr(label_buf.as_ptr())
         .to_str()
         .unwrap()
         .to_owned()
   }
      ...
      match label {
         Some(l) => {
            ...
            gl.ObjectPtrLabel(
               ...
               length,
               lbl.as_ptr() as *const ngl::GLchar,
            );
         }
         None => gl.ObjectPtrLabel(sync.0 as *mut std::ffi::c_void, 0, ptr::null()),
      }
   }
   ...
   unsafe fn get_shader_storage_block_index(
      ...
      let gl = &self.raw;
      let name = CString::new(name).unwrap();
      let index = gl.GetProgramResourceIndex(program.0.get(), SHADER_STORAGE_BLOCK, name.as_ptr());
      if index == INVALID_INDEX {
         None
      } else {
         Some(index)
         ...
         Some(_) => {
            ...
            unsafe {
               ...
               if gl.DebugMessageCallback_is_loaded() {
                  ...
                  gl.DebugMessageCallbackKHR(None, ptr::null());
               }
            }
         }
         None => {}
